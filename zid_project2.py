...
def read_prc_csv(tic):
    ...
    TIC= os.path.join(cfg.DATADIR, f'{tic}_prc.csv')
    df = pd.read_csv(TIC)
    df['Date'] = pd.to_datetime(df['Date'])
    df.set_index('Date',inplace=True)
    df = cfg.standardise_colnames(df)
    return df
print(read_prc_csv('aal'))
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    for i in tickers:
        flag = read_prc_csv(i)
        df = pd.concat([df, flag[prc_col]], axis=1)
    df.columns = tickers
...
def mk_ret_df(prc_df):
    ...
    """
    mk_df = pd.read_csv(cfg.FF_CSV, index_col='Date')
    mk_df.index = pd.to_datetime(mk_df.index)
    ret_df = (prc_df - prc_df.shift(1)) / prc_df.shift(1)
    ret_df = ret_df.join(mk_df[['mkt']], how='left')
    return ret_df
...
def mk_aret_df(ret_df):
    ...
    """
    for col_1 in ret_df.columns:
        if col_1 != 'mkt':
            ret_df[col_1] = ret_df[col_1] - ret_df['mkt']
    ret_df.drop('mkt', axis=1, inplace=True)
    return ret_df
...
def get_avg(df, col, year):
    ...
    """
    tic = df[[col]].dropna()
    y = tic.groupby(tic.index.year).mean()
    avg = y.loc[year].iloc[0]
    return avg
...
def get_ann_ret(ser, start, end):
    ...
    """
    return ((ser[start:end] + 1).prod()) ** (252 / len(ser[start:end])) - 1
